...
from utils_new.misc import get_world_size
...
#     return losses
def trans_vg_loss_old(batch_pred, batch_target, batch_labels, pad_val=-1):#这是之前没有加入置信度时的loss
    """
    batch_pred: [B, num_queries, 4]
    batch_target: [B, num_queries, 4]
    batch_labels: [B, num_queries]
    pad_val: int, pad填充值
    """
    mask = (batch_labels != pad_val)    # [B, N]
    ...
    pred_flat = batch_pred[mask]    # [N, 4]
    tgt_flat  = batch_target[mask]  # [N, 4]

    # L1 loss
    loss_bbox = F.l1_loss(pred_flat, tgt_flat, reduction='sum') / num_valid

    # GIoU loss
    pred_xyxy = xywh2xyxy(pred_flat)
    tgt_xyxy  = xywh2xyxy(tgt_flat)
    giou = generalized_box_iou(pred_xyxy, tgt_xyxy)   # [N, N]
    # 只算每个框自己的giou（diag）
    loss_giou = 1 - torch.diag(giou)
    loss_giou = loss_giou.sum() / num_valid

    return {'loss_bbox': loss_bbox, 'loss_giou': loss_giou}



def trans_vg_loss(batch_pred, batch_target, batch_labels, pad_val=-1):
    """
    batch_pred: [B, num_queries, 5] (x, y, w, h, conf_logit)
    batch_target: [B, num_queries, 5]
    batch_labels: [B, num_queries]
    pad_val: int, pad填充值
    """
    mask = (batch_labels != pad_val)    # [B, N]
    ...
    ## Bbox & giou loss，只在真实目标上监督
    pred_box_flat = batch_pred[..., :4][mask]    # [N, 4]
    tgt_box_flat  = batch_target[..., :4][mask]  # [N, 4]
    # L1 loss
    loss_bbox = F.l1_loss(pred_box_flat, tgt_box_flat, reduction='sum') / num_valid
    # GIoU loss (你可能要确保xywh2xyxy函数已实现)
    pred_xyxy = xywh2xyxy(pred_box_flat)
    tgt_xyxy  = xywh2xyxy(tgt_box_flat)
    # generalized_box_iou输出[N, N], 需要取对角
    giou = generalized_box_iou(pred_xyxy, tgt_xyxy)
    loss_giou = 1 - torch.diag(giou)
    loss_giou = loss_giou.sum() / num_valid
    ## conf loss, 所有query都要监督（正样本gt conf=1, pad=0；见DETR/yolo做法）
    pred_conf = batch_pred[..., 4]
    tgt_conf  = batch_target[..., 4]
    # pred_conf通常是logit，直接用with_logits=TRUE
    pos = (tgt_conf == 1)
    neg = (tgt_conf == 0)
    ratio = neg.sum().float() / (pos.sum().float() + 1e-10)
    # 每个正样本loss乘以ratio
    loss_conf = F.binary_cross_entropy_with_logits(
        pred_conf, tgt_conf, reduction='none'
    )
    loss_conf = (loss_conf * (pos * ratio + neg)).mean()
    # loss_conf = F.binary_cross_entropy_with_logits(pred_conf, tgt_conf, reduction='mean')

    return {'loss_bbox': loss_bbox, 'loss_giou': loss_giou, 'loss_conf': loss_conf}
