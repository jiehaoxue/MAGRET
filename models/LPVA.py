...
class LPVA(nn.Module):
    def __init__(self, args):
        ...
        self.num_queries = 12  # 举例同时预测20个目标
        ...
        num_total = self.num_visu_token + self.num_text_token + self.num_queries
        self.visual_pos = nn.Embedding(self.num_visu_token,hidden_dim)

        # 多目标修改 self.reg_token = nn.Embedding(1, hidden_dim)
        self.reg_token = nn.Embedding(self.num_queries, hidden_dim)
        ...
        #self.bbox_embed = MLP(hidden_dim, hidden_dim, 4, 3)#这是没有加置信度之前的
    ...
    def forward(self, img_data, text_data):
        #print(f"Image input shape: {img_data.shape}")
        ...
        #Text source shape: torch.Size([16, 40, 768])意思是每个batch有16句话，每一句话有40个token，每个token有768维 40是max_query_len
        #text_mask shape: torch.Size([16, 40])掩码来告诉模型哪些token是padding的
        # print(f"Text source shape: {text_src.shape}")
        
        # texts=text_src[:,0,:]     #Texts shape: torch.Size([16, 768])
        texts=text_src 
        # print(f"Texts shape: {texts.shape}")
        texts = self.text_embd(texts) #Texts shape: torch.Size([16, 256]) 降维
        ...
        #多目标修改tgt_mask = torch.zeros((bs, 1)).to(tgt_src.device).to(torch.bool)
        tgt_mask = torch.zeros((bs, self.num_queries)).to(tgt_src.device).to(torch.bool)
        ...
        # 多目标修改vg_hs = vg_hs[0]
        vg_hs = vg_hs[:self.num_queries]
        pred_box = self.bbox_embed(vg_hs).sigmoid()

        pred_box = pred_box.permute(1, 0, 2)

        return pred_box#shape：[batch_size, num_queries, 4]，
