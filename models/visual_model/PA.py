...
class PA(nn.Module):
    ...
    def forward(self, x, language_features):
        ...
        #print("language_features.size"+str(language_features.size()))#language_features.sizetorch.Size([16,40, 256])
        language_features = self.avgpool(language_features.permute(0, 2, 1)).squeeze(-1)  # 计算全局语言特征
        # language_features = self.avgpool(language_features.permute(1,0))
        ef = language_features #ef shape: torch.Size([256, 1])
