...
def train_one_epoch(args, model: torch.nn.Module, data_loader: Iterable, 
                    ...
                    epoch: int, max_norm: float = 0, iter=0, writer=None):
    model.train()
    ...
    for batch in metric_logger.log_every(data_loader, print_freq, header):
        ...
        # img_data, img_mask,text_data, word_mask,target,label = batch
        ...
        text_data = text_data.to(device)
        #word_mask_tensor = torch.tensor(word_mask).to(device)
    
        # text_input = TextInput(text_data, word_mask_tensor)
        # img_input = NestedTensor(img_data, img_mask)
        # target= target.to(device)
        target = target.to(device)
        # print(f"Type of img_data.tensors: {type(img_data)}")
        # print(f"Shape of img_data.tensors: {img_data.shape}")
        ...
        loss_dict = loss_utils.trans_vg_loss(pred_box,target,label)



        losses = sum(loss_dict[k] for k in loss_dict.keys())
        loss_dict_all = {**loss_dict, 'total_loss': losses}
        writer.add_scalars('losses', loss_dict_all, iter)
    ...
    return {k: meter.global_avg for k, meter in metric_logger.meters.items()}, iter
...
def validate(args, model: torch.nn.Module, data_loader: Iterable, device: torch.device):
    ...
    for batch in metric_logger.log_every(data_loader, 10, header):
        ...
        # precision, recall, f1 = eval_utils.compute_metrics(pred_boxes,target)
        # metric_logger.update_v2('precision',  precision, batch_size)
        # metric_logger.update_v2('recall', recall, batch_size)
        # metric_logger.update_v2('f1_score', f1, batch_size)

        results, mean_iou, cum_iou = eval_utils.compute_metrics_multi_iou_conf(pred_boxes, target,conf_threshold=0.3)
        for thr, (prec, recall, f1) in results.items():
            metric_logger.update_v2(f'P@{int(thr*100)}', prec, batch_size)
            metric_logger.update_v2(f'R@{int(thr*100)}', recall, batch_size)
            metric_logger.update_v2(f'F1@{int(thr*100)}', f1, batch_size)
        metric_logger.update_v2('meanIoU', mean_iou, batch_size)
        metric_logger.update_v2('cumIoU', cum_iou, batch_size)
