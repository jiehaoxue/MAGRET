...
def main(args):
    ...
    #记录每次实验的日志，使用tensorboard可视化，每次做实验之前 python train.py --name xjh-514
    args.output_dir = os.path.join(args.output_dir, args.name)
    writer_dir = os.path.join(args.output_dir, 'logs')
...
#////////////////////////////////////////////////////////////////////
    ...
    if args.lr_scheduler == 'poly':
        ...
        raise ValueError('Lr scheduler type not supportted ')
    dataset_train = build_dataset('train_remove_single-target_xmls', args)
    dataset_val = build_dataset('val_remove_single-target_xmls', args)

    print('trainset:', len(dataset_train), 'validation set:', len(dataset_val))
    ...
    print("Multi_target data loaded\n--------------------------------------------------------------")
    if args.resume:
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        train_stats, iter = train_one_epoch(
            args, model, data_loader_train, optimizer, device, epoch, args.clip_max_norm, iter, writer=writer)
        ...
        log_stats = {'epoch': epoch,
                    **{f'train_{k}': v.item() if isinstance(v, torch.Tensor) else v for k, v in train_stats.items()},
