...
class RandomResize(object):#//////////////////这个用到了
...
class NormalizeAndPad(object):#//////////////////这个用到了
    ...
    def __call__(self, input_dict):
        ...
        if 'box' in input_dict.keys():
            ...
            box = xyxy2xywh(box)
            box = box / torch.tensor([w, h, w, h], dtype=torch.float32)#把4个数归一化到 [0,1] 区间：x/w, y/h, w/w, h/h（变成相对坐标）。
            input_dict['box'] = box
